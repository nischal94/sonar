...
> ⚠️ **Post-ship note (2026-04-20, session 7):** Despite this plan marking Task 13 as "wire Celery dispatch from trigger endpoint" and PR #77 shipping with all 14 tests green, the trigger endpoint was never actually wired. `POST /workspace/backfill/trigger` flipped `Workspace.backfill_used=True`, returned a synthesized `task_id`, and never called `.delay()`. First live dogfood surfaced this; [PR #104](https://github.com/nischal94/sonar/pull/104) completed the wiring by adding `backend/app/jobs/day_one_backfill_task.py` as the Celery task wrapper (pattern mirrors `app/jobs/public_poller.py`), registering it in `app/workers/celery_app.py`, and dispatching via `.delay(str(workspace_id))` from the router. Also resolved 6 other end-to-end bugs (MV3 CORS, LinkedIn DOM drift, Apify URL tracking params, Pydantic 422 on long names, etc.) — see [`docs/session-notes/2026-04-20-session-7.md`](../session-notes/2026-04-20-session-7.md) for the full forensic.
