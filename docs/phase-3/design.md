...
**Legal posture:** scraping public LinkedIn profiles is covered by *HiQ Labs v. LinkedIn* (9th Circuit, 2022 — Supreme Court declined cert). We are not scraping private feed content, we are not logged in as the user, we are not bypassing a paywall. Public data only. Legal risk is substantially lower than the extension's grey-zone "user's own session" defense.
...
CREATE TABLE targets (
  ...
  linkedin_url TEXT NOT NULL,           -- canonical form, used as dedup key
  ...
  segment_tag TEXT,                     -- user-provided: "enterprise", "mid-market", "icp-1"
  priority_weight REAL NOT NULL DEFAULT 1.0,
  added_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  last_scraped_at TIMESTAMPTZ,
  scrape_status TEXT NOT NULL DEFAULT 'pending',  -- pending / active / failed / paused
  scrape_failure_reason TEXT,
  fit_score REAL,                       -- cached; parallel to connection.fit_score
...
ALTER TABLE posts ADD COLUMN source TEXT NOT NULL DEFAULT 'extension';  -- 'extension' | 'target_scrape' | 'manual_flag'
```

Every ingest path stamps `posts.source`, giving us forever-traceable provenance.
...
4. **Fit × Intent DoD still holds on target-scraped data:** P@5 ≥ 0.6, Recall@5 ≥ 0.5, zero top-5 competitors on at least two labeled workspaces. Calibration redone against target-scraped posts (not feed-observed ones).
5. **Weekly Apify cost ≤ $50/workspace** at a 500-target list with daily cadence. If higher, revisit scrape cadence.
...
1. Migration 012 (targets table) + 013 (posts.target_id, posts.source) + ORM updates
2. `app/models/target.py` + basic target CRUD endpoints (`POST/GET/DELETE /workspace/targets`)
3. `app/services/target_scraper.py` — Protocol + RealTargetScraper (Apify-backed) + FakeTargetScraper + DI factory
4. `app/workers/scrape_targets_daily.py` — Celery beat task; processes one workspace at a time; status tracking on targets
5. Pipeline extension — when `posts.target_id` is set and `posts.source='target_scrape'`, use `target.fit_score` instead of `connection.fit_score`. Reuse `compute_fit_score`, `compute_hybrid_score`.
6. Wizard steps 3 + 4 — target paste/upload UI + scrape progress view
7. Dashboard redesign — ranked target activity + segment grouping + drill-in
8. CSV import script + `POST /workspace/targets/csv-upload` endpoint
9. Extension deprecation banner in frontend + onboarding skip-extension flow
10. Calibration redo — generate a labeled target-scraped dataset, rerun `analyze-hybrid`, validate DoD holds on the new data distribution
11. v3.0 launch gate: flip a new feature flag `workspace.use_target_based_ingest=TRUE` per workspace after successful onboarding; monitor for 2 weeks before full default

Then v3.1 (CRM sync), v3.2 (extension deprecation), v3.3 (extension-as-overlay or removal).
